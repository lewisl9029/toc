...
(function(process) {
  ...
  AdvancedOptimizer.prototype.mergeAdjacent = function(tokens) {
    ...
    for (var i = 0,
      ...
      if (lastToken.kind == 'selector' && token.metadata.selector == lastToken.metadata.selector) {
        ...
        changeBodyOf(lastToken, this.propertyOptimizer.process(token.value, lastToken.body.concat(token.body), joinAt, true));
        forRemoval.push(i);
      } else if (lastToken.body && token.metadata.body == lastToken.metadata.body && !this.isSpecial(token.metadata.selector) && !this.isSpecial(lastToken.metadata.selector)) {
        changeSelectorOf(lastToken, CleanUp.selectors(lastToken.value.concat(token.value), false, adjacentSpace));
        forRemoval.push(i);
      } else {
        lastToken = token;
      }
  ...
  AdvancedOptimizer.prototype.reduceNonAdjacent = function(tokens) {
    ...
    for (var i = tokens.length - 1; i >= 0; i--) {
      ...
      var selectors = isComplexAndNotSpecial ? [token.metadata.selector].concat(token.metadata.selectorsList) : [token.metadata.selector];
      for (var j = 0,
  ...
  AdvancedOptimizer.prototype.mergeNonAdjacentByBody = function(tokens) {
    ...
    for (var i = tokens.length - 1; i >= 0; i--) {
      ...
      if (oldToken && !this.isSpecial(token.metadata.selector) && !this.isSpecial(oldToken.metadata.selector)) {
        changeSelectorOf(token, CleanUp.selectors(oldToken.value.concat(token.value), false, adjacentSpace));
        changeBodyOf(oldToken, {
          tokenized: [],
          list: []
        });
  ...
  function optimizeProperties(tokens, propertyOptimizer) {
    for (var i = 0,
      ...
      var token = tokens[i];
      if (token.kind == 'selector') {
        changeBodyOf(token, propertyOptimizer.process(token.value, token.body, false, true));
