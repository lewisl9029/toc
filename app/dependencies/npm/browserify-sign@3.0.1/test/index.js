...
(function(Buffer, process) {
  ...
  fixtures.invalid.verify.forEach(function(f) {
    test(f.description, function(t) {
      ...
      var pub = new Buffer(f.public, 'base64');
      var message = new Buffer(f.message);
      t.notOk(nodeCrypto.createVerify(f.scheme).update(message).verify(pub, sign), 'node rejects it');
      t.notOk(myCrypto.createVerify(f.scheme).update(message).verify(pub, sign), 'We reject it');
    });
  });
